...
public class MemberServiceImpl implements MemberService{
	...
	public Member memberCheckWithNameandEmail(Member memberNeedsNameandPass) {
		return memberDAO.memberCheckWithNameandEmail(memberNeedsNameandPass);
	}
	public int changePassword(Member member) throws EditFailException{
		int result=memberDAO.changePassword(member);
		if(result==0) {
			throw new EditFailException("회원정보 수정에 실패하셨습니다.");
		}
		return result;
	}
	...
	public String registCheckWithEmail(String email) {
		return memberDAO.registCheckWithEmail(email);
	}
