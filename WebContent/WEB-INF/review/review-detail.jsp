...
function report(reviewTitle,alcoholName){
	window.open("/report?reviewTitle="+reviewTitle+"&alcoholName="+alcoholName+"","window팝업","width=600px, height=300px, top=250px, left=250px, menubar=no, toolbar=no");
}
</script>
</head>
<body>
						...
						<div class="list-group">
							...
							<div class="row">
								...
								<div class="col-md-2 text-center">
									...
									<button type="button" class="btn btn-primary btn-lg btn-block"
										<%if (loginMember == null) {%> onClick="loginAlert()"
										<%} else {%> onClick="report('<%=review.getTitle()%>','<%=alcohol.getName()%>')" <%}%>>신고하기</button>
								</div>
							</div>
							<hr>
